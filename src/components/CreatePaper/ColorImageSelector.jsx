...
const Button = styled(ButtonOrigin)`
  background: ${({ $toggle }) =>
    $toggle === 'color' ? `var(--orange-200, #FFE2AD)` : `url(${desert})`};
  background-size: cover; //오리진으로 올릴시 백그라운드 속성 순위에 밀려서 적용되지않음
`;

const Button2 = styled(ButtonOrigin)`
  background: ${({ $toggle }) =>
    $toggle === 'color' ? `var(--purple-200, #ECD9FF)` : `url(${basket})`};
  background-size: cover;
`;

const Button3 = styled(ButtonOrigin)`
  background: ${({ $toggle }) =>
    $toggle === 'color' ? `var( --Blue-200, #B1E4FF)` : `url(${mokoko})`};
  background-size: cover;
`;

const Button4 = styled(ButtonOrigin)`
  background: ${({ $toggle }) =>
    $toggle === 'color' ? `var(--Green-200, #D0F5C3)` : `url(${sky})`};
  background-size: cover;
`;

export default function ColorImageSelector({ toggle }) {
  const { color, setColor } = useContext(LocaleContext);
  return (
    <Wrap>
      <Button
        type="button"
